...
// La clé est la ligne de labels déjà formatée (ex: path="/api/order",code="200").
type counter struct {
	mu     sync.Mutex
	name   string
	help   string
...
var (
	...
	authzDecisions = newCounter("authz_decisions_total", "Décisions d'autorisation des appels relayés.")
)
...
// instrument enveloppe un handler pour compter ses requêtes. Le statut de la
// réponse relayée est capturé via un ResponseWriter qui le mémorise.
func instrument(path string, h http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		sw := &statusWriter{ResponseWriter: w, code: http.StatusOK}
		h(sw, r)
		recordRequest(path, sw.code)
		// Côté gateway, une réponse 2xx d'un backend = appel autorisé ; un 403
		// (refus de la politique par route) ou une erreur = refusé.
		recordAuthz(sw.code < 300)
	}
}
